#!/usr/bin/python3
import sys, hashlib
from sympy import *
from Crypto.Util.number import getPrime, inverse, bytes_to_long, long_to_bytes, GCD


# using big value of e cause that d would be enought small that allow to use Wiener's attack.
# I used information for Wiener attack from: https://sagi.io/2016/04/crypto-classics-wieners-rsa-attack/ and based on that codes I built solution for this task.


n = 
e = 
ct = 


def cf_expansion(n, d):
    e = []

    q = n // d
    r = n % d
    e.append(q)

    while r != 0:
        n, d = d, r
        q = n // d
        r = n % d
        e.append(q)

    return e


def convergents(e):
    n = [] # Nominators
    d = [] # Denominators

    for i in range(len(e)):
        if i == 0:
            ni = e[i]
            di = 1
        elif i == 1:
            ni = e[i]*e[i-1] + 1
            di = e[i]
        else:
            ni = e[i]*n[i-1] + n[i-2]
            di = e[i]*d[i-1] + d[i-2]

        n.append(ni)
        d.append(di)
        yield (ni, di)


if __name__ == '__main__':
    cf_expansion_ = cf_expansion(e, n)
    convergents_ = convergents(cf_expansion_)
    for pk, pd in convergents_: 
        if pk == 0:
            continue

        possible_phi = (e*pd - 1)//pk

        p = Symbol('p', integer=True)
        roots = solve(p**2 + (possible_phi - n - 1)*p + n, p)

        if len(roots) == 2:
            pp, pq = roots # pp - possible p, pq - possible q
            if pp*pq == n:
                d = inverse(e,possible_phi)
                pt = pow(ct, d, n)
                decrypted = long_to_bytes(pt)
                print(decrypted)


